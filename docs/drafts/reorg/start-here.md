...
Flow metrics under this model are descriptive statistics over element-wise distributions: averages, percentiles, etc. of arrivals, departures, process times etc. over business-facing reporting periods (weeks, months, quarters), or aligned with some business process cadence (sprints, releases). The element-boundary model implicitly assumes that elements are the primary observable units, and that process behavior is described by statistical distributions over observed element-wise trajectories through the process. This model is a direct port of measurement techniques used in production processes, and it has the benefit of being relatively intuitive to practitioners and managers.

The model is effective when the underlying processes are standardized repetitive tasks, with pre-determined process boundaries and routes. In many operational settings, process times can be bounded tightly a priori even if they are not all standardized. More often than not, such processes are stable or can be stabilized with focused effort.

When applied to _stable_ processes, these element-wise metrics have the added benefit that they map naturally to quantities that are governed by Little's Law. Stability here means that time-normalized quantities (arrival rate, throughput, and average process time) converge or remain approximately constant as the observation horizon grows. Under these conditions, the relationships between these quantities satisfy Little’s Law.

Under stable conditions governed by Little's Law, the model allows us to design policies that set arrival rates and WIP levels to achieve desired throughput and process time targets, and to reason about both individual processes and process networks. In other words, creating stability allows us to design and actively manage processes that meet business objectives using these metrics, rather than simply use them as reporting artifacts.
...
- Processes that operate in open systems,
- Highly variable element-wise trajectories,
- Finite but time-varying system capacity that is hard to measure directly,
- Non-stationary distributions of key metrics,
- Internal dependencies and adaptive feedback effects that impact flow dynamics,
- Frequent transitions between equilibrium states, and
- Long-lived "transients".

Under such non-stationary, non-equilibrium operating conditions, element-level descriptions are not the right primitive for reasoning about the structural dynamics of flow. They obscure essential analytical machinery and the incidental complexity they introduce does not help us much for the task at hand. In fact, it is easier to reason about flow in these situations with simpler representations. In other words, we can address a more complex problem by simplifying the representation of the arrival-departure process, rather than attempting to model its full complexity directly.

Our position is that rigorous techniques to measure and analyze these phenomena are more important than collecting and reporting descriptive statistics designed for stable processes. The techniques we will describe here let us do that without modeling individual elements, domain topology, or element trajectories.

To do this, we turn instead, to a model that lets us reason rigorously about process dynamics while making fewer assumptions about elements, boundaries, routes, and related structure.
...
In this model, an arrival-departure process is represented as a sequence of events on a shared timeline. Formally, it is a sequence of timestamps with a mark indicating whether each timestamp represents an arrival or a departure. Mathematically, this is called a marked point process. This representation gives us the minimal observable structure: a sequence of arrival and departure events over time.

We do not require that the events in [@fig:arrival-departure-process] be associated with elements, that structural boundaries are fixed in advance, or that explicit correspondence between arrival and departure events is established — for example, by matching them through an item identifier.

Rather, we assume the weaker requirements that every departure corresponds to an arrival that precedes it in time, and that for every observed departure event, there _exists_ a corresponding arrival event, which we may or may not be able to observe. These weaker assumptions are better suited for reasoning about flow in dynamic environments in which the internal mechanisms that generate these events may change over time. Element trajectories in such systems may also vary widely.

Events alone are not yet flow metrics; they determine how the state of the process evolves over time. Flow metrics are derived from this evolving state.

Let's motivate this distinction with a couple of examples.

### Example 1: Population Dynamics

To study population dynamics we record births and deaths, which we treat as arrival and departure events. We can measure population in units of people and lifespans in units of time without needing to track individuals from birth to death or establish explicit correspondence between specific births and deaths.

If we collect data over timescales that are much longer than the typical human lifespan, birth/death rates, average population, and average lifespan will stabilize, letting us use Little's Law to reason about the relationships between rates and averages without identifying individuals or needing to know anything about statistical distributions of these metrics.

### Example 2: Traffic networks

As another example closer to the kinds of operational problems we will study, consider the analysis of flow and congestion in a traffic network or sub-network. Many useful questions about traffic flow and congestion can be analyzed without knowing the topology of the network or the detailed trajectories of the vehicles moving through it.

Assuming that entrances and exits to and from the network are instrumented and we can count these arrivals and departures, we can reason about key flow metrics like arrival rates, throughput, average transit times, and congestion without tracking individual vehicle paths. The network topology can be iteratively decomposed to perform these analyses at different levels of detail, without ever needing to analyze individual element trajectories.
...
In stable processes, the two models align in that the metrics they produce agree on average in common use cases; in processes that operate far from stable equilibrium, or where more nuanced definitions of process state are required, the marked point process model and the Presence Calculus offer more precise tools for reasoning about flow because they rely on fewer modeling assumptions.

The marked point process representation is to flow analysis what a single-celled organism is to biology: simple enough to exhibit the core principles, yet rich enough to support the analytical machinery to measure flow dynamics. We begin here because every structural property needed to study more complex configurations, including element-boundary models, can be derived from this case. Understanding it well is the foundation for everything that follows.

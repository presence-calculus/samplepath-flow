...
All these complex, interdependent concerns are bundled into an amorphous, underspecified concept called "flow." Exactly what "flow" is is never defined, but it is implied that improving it will improve business outcomes. The causal mechanisms that lead from one to the other are never substantiated. Entire edifices — process frameworks, measurement systems, tooling and theories — have been built up in the industry over the last 15 years, without a closer examination of the underlying premises, or theoretical validity of their foundational assumptions. In this aspect, "flow" is very much like that other overused but underspecified concept, productivity.

Our first objective in this document is to shore up those foundations in the case of "flow." Doing so reveals where current practices fit within the broader mathematical context provided by a rigorous theory of stochastic processes. It helps us understand where and when certain approaches are appropriate, and whether a particular improvement method is likely to be effective before implementation.
...
![The Element-Boundary Model]($document-root/assets/element-boundary-model.png){#fig:element-boundary}

The standard approach to flow analysis under this model is to study the distributional properties of these element-wise metrics (averages, percentiles, etc.) over business-facing reporting periods (weeks, months, quarters), or aligned with some business process cadence (sprints, releases). The element-boundary model implicitly assumes that elements are the primary observable units, and that process behavior can be characterized using statistical distributions over observed element trajectories. This model is a direct port of measurement techniques used in production processes, and it has the benefit of being relatively intuitive to practitioners and managers.

When applied to _stable_ processes, these element-wise metrics have the added benefit that they map naturally to quantities that are governed by Little's Law. It is generally much simpler to assume stable processes are the default when the underlying processes are standardized repetitive tasks, with pre-determined process boundaries and routes. In many operational settings, process times can be bounded tightly a priori. Stability here means that time-normalized quantities (arrival rate, throughput, and average process time) converge or remain approximately constant as the observation horizon grows. Under these conditions, these quantities are well-approximated by Little’s Law.

If we can assume that an arrival-departure process is stable, the element-boundary model becomes considerably more powerful. Across a single boundary, it allows us to design policies that set arrival rates and WIP levels to achieve desired throughput and process time targets, and to reason about both individual processes and process networks using Little’s Law. In other words, stability allows us to design processes that meet business objectives using these metrics, rather than simply use them as reporting artifacts.

But what if we cannot assume the arrival-departure process is stable?

This is the case in most collaborative knowledge work, including software development. Here we cannot always assume uniform elements, well-defined boundaries or routes, bounded process times, or stable arrival rates and process time distributions. It turns out it is simpler to reason about flow in these domains if we can do so without requiring us to first specify elements and boundaries. In other words, we address a more complex problem by simplifying the representation of the arrival-departure process, rather than attempting to model its full complexity directly.

The process model we use in The Presence Calculus is therefore simpler and more elementary than the standard element-boundary model we see in the software industry. It is more general in that it makes fewer assumptions about elements, and does not require the specification of boundaries, routes, and related structure.


## The Marked Point Process Model

![Arrival-Departure Marked Point Process]($document-root/assets/arrival-departure-process.png){#fig:arrival-departure-process}

In the Presence Calculus an arrival-departure process is represented using arrival and departure events on a shared timeline. Formally, it is simply a sequence of timestamps with a mark indicating whether the timestamp represents an arrival event or a departure event. Mathematically, this is called a marked point process.

We do not require that the events in [@fig:arrival-departure-process] be associated with elements, or that structural boundaries that determine how process time is calculated are fixed in advance. In particular, we do not require the model to establish explicit correspondence between arrival and departure events, for example by matching them through an item identifier.

Rather, we assume the weaker requirements that every departure is preceded by an arrival in time, and that for every observed departure event, there _exists_ a corresponding arrival event, which we may or may not be able to observe. These weaker assumptions, free of element and boundary constraints, are more suitable for reasoning about flow in dynamic environments in which the internal mechanisms that generate these events may change over time and element trajectories may vary widely.

Let's motivate this distinction with a couple of examples.

### Example 1: Births, Deaths and Population Dynamics

To study population dynamics we need to record births and deaths which we treat as arrival and departure events. We can measure population in units of people and lifespans in units of time. We can do all this without needing to track individuals from birth to death. Changes in population, trends in births, deaths, and lifespan, etc. can be measured without needing explicit correspondence between a specific birth and a specific death, or a single integrated record of both across individual life trajectories.

If we collect data over timescales that are much longer than the typical human lifespan, birth/death rates, average population, and average lifespan will stabilize, letting us use Little's Law to reason about the relationships between rates and averages without identifying individuals or knowing anything about statistical distributions of these metrics.


### Example 2: Traffic networks

As another example much closer to the kinds of operational problems we will be studying, consider the analysis of flow and congestion in a traffic network or sub-network. Many useful questions about traffic flow and congestion of a network or sub-network can be analyzed without knowing anything about the topology of the network or the detailed trajectories of the vehicles moving through it.

Assuming that entrances and exits to and from the network are instrumented and we can count these arrivals and departures, we can reason about key flow metrics like arrival rates, throughput, average transit times, congestion, etc., without needing to track the individual paths or trajectories of vehicles in the network. The network topology can be iteratively decomposed to perform these analyses at different levels of detail, without ever needing to drop down to the level of analyzing individual vehicle paths.

The traffic network example has many similarities to the kinds of flow analysis problems we encounter in digital operations management: processes that operate in open systems, highly variable element-wise trajectories, finite but time-varying system capacity that is hard to quantify accurately, non-stationary distributions of key metrics, internal dependencies and adaptive feedback effects between elements traversing the network that impact flow dynamics, frequent transitions between equilibrium states with measurable long-lived transient states, among others.

Our position is that it is much more important to have rigorous techniques to measure and analyze these phenomena rather than simply collecting and reporting metrics that are designed for stable processes. The techniques we will describe here let us do that without needing to model individual elements, domain topology or element trajectories. Formalizing the dividing line between the kinds of questions about flow that we can answer with the minimal marked point process models and the ones that need the additional structure of the element-boundary model may be considered the main focus of this document. The simpler model of marked point processes combined with the measurement techniques of the Presence Calculus provide new tools to reason about flow in processes that operate within the volatile, uncertain and complex systems we face in modern digital operations management.

In the Presence Calculus, "flow" and "flow metrics" are properties derived from the *state* of an arrival-departure process that evolves over time. The Presence Calculus gives us the tools to define and reason about process state starting from a purely time- and event-based representation of an arrival-departure process. This cleanly separates flow analysis from the specific structures and mechanisms that generate arrival and departure events, and allows us to measure process dynamics rigorously even when the underlying processes are non-stationary. All we require is that there exists a stable notion of what constitutes an arrival or departure event in a domain, and that these events can be observed and recorded on a common timeline.

The main assumption we make is that arrival and departure events are *countable* and that arrival and departure counts are expressed in compatible units. In this sense, the model indirectly presupposes an underlying discrete set of elements over which these counts are defined, but we do not require correspondence or traceability between events and elements — only that arrival and departure *counts* are commensurate. There are additional technical assumptions, which we will introduce explicitly in context as we explore specific aspects of process dynamics.

In short, everything needed to analyze the dynamics of flow is determined entirely by the arrival and departure events.

The element-boundary model remains important and the general form of the Presence Calculus we describe in [@kkumar2025-pc-intro] supports this. However, deriving flow principles from the strictly weaker assumptions of the marked point process model allows us to see precisely where element and boundary structure become necessary in flow analysis. In stable processes, these two models are equivalent in the sense that the metrics they produce agree on average; in processes that operate far from stable equilibrium, however, the marked point process model is a fundamentally superior tool for reasoning about flow because it makes fewer modeling assumptions.

The marked point process representation is to flow analysis what a single-celled organism is to biology: simple enough to exhibit the core principles, yet rich enough to support the full analytical machinery to describe and measure flow dynamics. We begin here because every structural property needed to study more complex configurations, including element-boundary models, can be derived from this case. Understanding it well is the foundation for everything that follows.

# Sample Path Analysis

We now come to sample path analysis, the second main way in which our approach to measuring flow differs from current statistically based methods. The key point here is that this is a technique that lets us define _a deterministic measurement model over non-deterministic processes._ Let's define what this means.
...
These results are particularly valuable in precisely those non-steady-state conditions where the steady-state form of Little’s Law provides little insight. [@fig:presence-invariant] shows the relationships between the processes that determine how flow metrics over a sample path behave as it evolves over time.

![The Presence Invariant]($document-root/assets/presence-invariant.png){#fig:presence-invariant}

In the sections below we will use these relationships to derive deterministic rules that explain how the metrics evolve in response to arrival and departure events. Together these rules provide a completely deterministic accounting of what we measure with flow metrics, free of any additional stochastic assumptions beyond that is what is already reified in the realized sample path.

...
from samplepath.metrics import compute_sample_path_flow_metrics
...
def single_boxcar():
    ...
    times = [t0, t1]
    return compute_sample_path_flow_metrics(events, times)


def test_single_boxcar_shapes(single_boxcar):
    _, L, Lam, w, N, H, Arr, Dep, w_prime, Theta = single_boxcar
    assert all(len(x) == 2 for x in (L, Lam, w, N, H, Arr, Dep, w_prime, Theta))


def test_single_boxcar_initial_values(single_boxcar):
    _, L, Lam, w, _, _, _, _, w_prime, Theta = single_boxcar
    assert (
        np.isnan(L[0])
        and np.isnan(Lam[0])
        and np.isclose(w[0], 0.0)
        and np.isnan(w_prime[0])
        and np.isnan(Theta[0])
    )


def test_single_boxcar_initial_counts(single_boxcar):
    _, _, _, _, N, H, Arr, Dep, _, _ = single_boxcar
    assert N[0] == 1 and np.isclose(H[0], 0.0) and Arr[0] == 1 and Dep[0] == 0
...
def test_single_boxcar_final_L(single_boxcar):
    _, L, _, _, _, _, _, _, _, _ = single_boxcar
...
def test_single_boxcar_final_counts(single_boxcar):
    _, _, _, _, N, _, Arr, Dep, _, _ = single_boxcar
...
def test_overlapping_items_N_sequence(overlapping_items):
    _, _, _, _, N, _, _, _, _, _ = overlapping_items
...
def test_overlapping_items_final_w_equals_A_over_arrivals(overlapping_items):
    _, _, _, w, _, H, Arr, _, _, _ = overlapping_items
    assert np.isclose(w[-1], H[-1] / Arr[-1])
...
def test_same_timestamp_tie_final_counts(same_timestamp_tie):
    _, _, _, _, N, _, Arr, Dep, _, _ = same_timestamp_tie
    assert N[0] == 0 and Arr[0] == 1 and Dep[0] == 1


def test_same_timestamp_tie_nan_metrics(same_timestamp_tie):
    _, L, Lam, w, _, H, _, _, w_prime, Theta = same_timestamp_tie
    assert (
        np.isnan(L[0])
        and np.isnan(Lam[0])
        and np.isclose(H[0], 0.0)
        and np.isclose(w[0], 0.0)
        and np.isclose(w_prime[0], 0.0)
        and np.isnan(Theta[0])
    )
...
def test_empty_events_returns_empty_arrays():
    ...
    for arr in (L, Lam, w, N, H, Arr, Dep, w_prime, Theta):
        assert arr.size == 0

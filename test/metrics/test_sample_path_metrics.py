...
def test_overlapping_items_final_area(overlapping_items):
    T, _, _, _, _, H, _, _ = overlapping_items
    t0, t1, t2, t3 = T
    expected_area = (
...
def test_overlapping_items_final_L_equals_A_over_elapsed(overlapping_items):
    T, L, _, _, _, H, _, _ = overlapping_items
...
def test_overlapping_items_final_w_equals_A_over_arrivals(overlapping_items):
    _, _, _, w, _, H, Arr, _ = overlapping_items
...
def test_same_timestamp_tie_nan_metrics(same_timestamp_tie):
    _, L, Lam, w, _, H, _, _ = same_timestamp_tie
    assert (
        np.isnan(L[0])
        and np.isnan(Lam[0])
...
def test_empty_events_returns_empty_arrays():
    ...
    times = [t0]
    events = []
    T, L, Lam, w, N, H, Arr, Dep = compute_sample_path_metrics(events, times)
    for arr in (L, Lam, w, N, H, Arr, Dep):
        assert arr.size == 0
